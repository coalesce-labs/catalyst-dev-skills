...
foreground work for CPU or I/O), `OnCalendar=daily` with `Persistent=true` (a missed run — machine was
off — still fires at next boot) and a jitter-derived `RandomizedDelaySec=`. `ExecStart` invokes this
...
niceness. `StartCalendarInterval` carries the jitter-derived minute.

**cron** — exactly one line, between `# BEGIN catalyst-prune-worktrees` / `# END catalyst-prune-worktrees`
markers so a re-install replaces only that block and leaves the rest of the crontab untouched.
