...
1. **Is this class already decided or already routed?** — ⭐ NEW. Check catalyst-cloud `.agents/references/standing-decisions.md`: a decision made once for this class is executed, not re-asked. Then a repeating generated failure has a standard action that depends on its **ladder verdict** (`references/validate-failing.md`); most of its sub-shapes are not human decisions at all.
...
- **A system-level failure is never a per-ticket human block** (one fleet alert, not N asks), and **"the runner refused to re-run" is not a decision** — 0 of 9 head-unchanged holds ever cleared on their own, and 3 of 3 humans who answered added no information.
