...
# isReplicaFresh): the writer heartbeat `<db>.writer.lock` must be recent AND a
# change-feed cursor must be present (proves the seed is complete, not mid-reseed).
...
#   • a non-empty change-feed cursor exists (seed complete, not mid-reseed): a
#     `sync_cursors` row, or the `sync_meta` 'cursor' row older writers keep.
...
replica_fresh() {
  ...
  (( age <= thr )) || return 1
  [[ -n "$(sqlite3 "$db" "SELECT 1 FROM sync_cursors WHERE cursor IS NOT NULL AND cursor<>'' LIMIT 1;" 2>/dev/null)" ]] \
