...
3. **Act on the recorded next step**, within the work the handoff already scoped. Do not widen scope because no one is there to object. First re-arm whatever the handoff's `Re-arm:` line lists (loops, wakeups, monitors, background tasks); the reset killed them.
